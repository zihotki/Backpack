...
namespace Zipkin
{
	public class ZipkinBootstrapper
	{
		...
		public static ZipkinBootstrapper BuildFromAppDomainConfig()
		{
			...
			if (serviceName == null)
			{
				throw new Exception("Missing configuration entry: 'ZipkinServiceName' under appsettings, which should define this service's name");
			}

			if (zipkinServer == null)
			{
				throw new Exception("Missing configuration entry: 'ZipkinServerName' under appsettings, which should point to zipkin server's hostname");
			}
		...
		public static void ChangeSamplingRate(double newRate)
		{
			...
			{
				throw new ArgumentOutOfRangeException(nameof(newRate), "sample rate should be between 0.0 and 1.0");
			}

			ZipkinConfig.SampleRate = newRate;
		}
		...
		public ZipkinBootstrapper WithSampleRate(double sampleRate)
		{
			...
			ZipkinConfig.SampleRate = sampleRate;
		...
		public void Start()
		{
			...
			Backpack.AddCleaner(new ZipkinTraceBackpackCleaner());
